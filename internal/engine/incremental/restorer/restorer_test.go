...
func saveTree(t *testing.T, ctx context.Context, repo *repository.Repository, tr *tree.Tree) incremental.ID {
	t.Helper()
	doc, err := tr.Marshal()
	require.NoError(t, err)
...
func TestRestoreRejectsUnsafeSnapshotTree(t *testing.T) {
	t.Run("symlink with subtree", func(t *testing.T) {
		...
		subID := saveTree(t, ctx, repo, &tree.Tree{Nodes: []*tree.Node{
			{Name: "pwn", Type: tree.TypeFile, Mode: 0o644, Content: []incremental.ID{}},
		}})
	...
	t.Run("name with path separator", func(t *testing.T) {
		...
		repo := newTestRepository(t)
		subID := saveTree(t, ctx, repo, &tree.Tree{Nodes: []*tree.Node{
			{Name: "nested/pwn", Type: tree.TypeFile, Mode: 0o644, Content: []incremental.ID{}},
		}})
		rootID := saveTree(t, ctx, repo, &tree.Tree{Nodes: []*tree.Node{
...
func TestRestoreSkipsSpecialNodes(t *testing.T) {
	...
	repo := newTestRepository(t)
	subID := saveTree(t, ctx, repo, &tree.Tree{Nodes: []*tree.Node{
		{Name: "f.txt", Type: tree.TypeFile, Mode: 0o644, Content: []incremental.ID{}},
...
func TestRestoreMissingBlobFails(t *testing.T) {
	ctx := context.Background()
	repo := newTestRepository(t)
	missing := incremental.Hash([]byte("never stored"))
	subID := saveTree(t, ctx, repo, &tree.Tree{Nodes: []*tree.Node{
		{Name: "f.txt", Type: tree.TypeFile, Mode: 0o644, Content: []incremental.ID{missing}},
	}})
	rootID := saveTree(t, ctx, repo, &tree.Tree{Nodes: []*tree.Node{
		{Name: "data", Type: tree.TypeDir, Mode: 0o755, Subtree: &subID},
	}})
