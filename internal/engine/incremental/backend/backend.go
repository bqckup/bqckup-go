...
type Backend interface {
	...
	Save(ctx context.Context, h incremental.Handle, rd io.Reader) error
	// Load reads up to length bytes from offset and calls fn with the reader.
	// A length of 0 reads to the end of the file.
	Load(ctx context.Context, h incremental.Handle, length int, offset int64, fn func(rd io.Reader) error) error
	// Stat returns file information, or an error for which IsNotExist is true.
	Stat(ctx context.Context, h incremental.Handle) (FileInfo, error)
	// List calls fn for every file of the given type.
	List(ctx context.Context, t incremental.FileType, fn func(h incremental.Handle, size int64) error) error
	// Remove deletes one file. Removing a missing file is not an error.
	Remove(ctx context.Context, h incremental.Handle) error
