...
func (b *S3) keyFor(h incremental.Handle) (string, error) {
...
func (b *S3) Save(ctx context.Context, h incremental.Handle, rd io.Reader) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	key, err := b.keyFor(h)
...
func (b *S3) Load(ctx context.Context, h incremental.Handle, length int, offset int64, fn func(rd io.Reader) error) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	key, err := b.keyFor(h)
...
func (b *S3) Stat(ctx context.Context, h incremental.Handle) (FileInfo, error) {
	if err := ctx.Err(); err != nil {
		return FileInfo{}, err
	}
	key, err := b.keyFor(h)
...
func (b *S3) List(ctx context.Context, t incremental.FileType, fn func(h incremental.Handle, size int64) error) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	var dir string
	...
	for {
		...
		for _, object := range out.Contents {
			...
			if err := fn(incremental.Handle{Type: t, Name: name}, aws.ToInt64(object.Size)); err != nil {
				return err
			}
		}
...
func (b *S3) Remove(ctx context.Context, h incremental.Handle) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	key, err := b.keyFor(h)
