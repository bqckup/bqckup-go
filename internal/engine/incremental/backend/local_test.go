...
func dataHandle(name string) incremental.Handle {
	return incremental.Handle{Type: incremental.DataFile, Name: name}
}
...
func TestSaveLoadRoundTripAllTypes(t *testing.T) {
	...
	handles := map[incremental.FileType]incremental.Handle{
		incremental.ConfigFile:   {Type: incremental.ConfigFile},
		incremental.KeyFileType:  {Type: incremental.KeyFileType, Name: longName(1)},
		incremental.LockFile:     {Type: incremental.LockFile, Name: longName(2)},
		incremental.SnapshotFile: {Type: incremental.SnapshotFile, Name: longName(3)},
		incremental.IndexFile:    {Type: incremental.IndexFile, Name: longName(4)},
		incremental.DataFile:     dataHandle(longName(5)),
	}
...
func TestListAndRemove(t *testing.T) {
	...
	for _, name := range keys {
		h := incremental.Handle{Type: incremental.KeyFileType, Name: name}
		if err := b.Save(ctx, h, strings.NewReader("x")); err != nil {
			t.Fatal(err)
		}
	}
	listed := map[string]int64{}
	err := b.List(ctx, incremental.KeyFileType, func(h incremental.Handle, size int64) error {
		listed[h.Name] = size
		return nil
	})
	if err != nil {
	...
	removed := incremental.Handle{Type: incremental.KeyFileType, Name: keys[0]}
...
func TestListMissingDirsIsEmpty(t *testing.T) {
	b := NewLocal(t.TempDir())
	count := 0
	if err := b.List(context.Background(), incremental.DataFile, func(incremental.Handle, int64) error {
		count++
		return nil
	}); err != nil {
		t.Fatal(err)
...
func TestInvalidHandlesRejected(t *testing.T) {
	...
	if err := b.Save(context.Background(), incremental.Handle{Type: incremental.FileType("bogus"), Name: "x"}, strings.NewReader("x")); err == nil {
		t.Fatal("want error for unknown file type")
	}
	if err := b.Save(context.Background(), incremental.Handle{Type: incremental.KeyFileType}, strings.NewReader("x")); err == nil {
