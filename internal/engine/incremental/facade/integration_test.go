...
import (
	...
	"github.com/bqckup/bqckup-go/internal/engine/incremental/facade"
...
func TestDisposableIncrementalBackupS3Compatible(t *testing.T) {
	...
	repoConfig := backupincremental.RepoConfig{
	...
	spec := backupincremental.BackupSpec{
		SiteName: siteName,
...
func removeRepository(ctx context.Context, repoBackend backend.Backend) error {
	for _, fileType := range []incremental.FileType{
		incremental.LockFile,
		incremental.SnapshotFile,
		incremental.IndexFile,
		incremental.DataFile,
		incremental.KeyFileType,
		incremental.ConfigFile,
	} {
		var handles []incremental.Handle
		if err := repoBackend.List(ctx, fileType, func(handle incremental.Handle, _ int64) error {
