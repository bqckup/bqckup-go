...
func (r *Repository) LoadBlob(ctx context.Context, blobType incremental.BlobType, id incremental.ID) ([]byte, error) {
	return r.loadBlob(ctx, blobType, id)
}

// LoadTree loads and parses one tree blob.
func (r *Repository) LoadTree(ctx context.Context, id incremental.ID) (*tree.Tree, error) {
	data, err := r.LoadBlob(ctx, incremental.TreeBlob, id)
	if err != nil {
...
func (r *Repository) LoadSnapshot(ctx context.Context, id incremental.ID) (SnapshotWithID, error) {
	return r.loadSnapshot(ctx, incremental.Handle{Type: incremental.SnapshotFile, Name: id.String()})
}

// DefaultPackSize is the pack flush threshold (16 MiB), matching incremental.
...
func (r *Repository) SaveBlob(ctx context.Context, blobType incremental.BlobType, data []byte) (incremental.ID, error) {
	if err := ctx.Err(); err != nil {
		return incremental.ID{}, err
	}
	if blobType != incremental.DataBlob && blobType != incremental.TreeBlob {
		return incremental.ID{}, fmt.Errorf("repository: invalid blob type %d", blobType)
	}
	...
	var entryType incremental.BlobType
	switch blobType {
	case incremental.DataBlob:
		packer = r.dataPacker
		if uncompressedLength > 0 {
			entryType = incremental.CompressedDataBlob
		} else {
			entryType = incremental.DataBlob
		}
	case incremental.TreeBlob:
		packer = r.treePacker
		if uncompressedLength > 0 {
			entryType = incremental.CompressedTreeBlob
		} else {
			entryType = incremental.TreeBlob
		}
	}
	...
	entry := index.Entry{
		Type:               blobType,
		ID:                 id,
		PackID:             incremental.ID{}, // filled in when the pack is flushed
...
func (r *Repository) Flush(ctx context.Context) error {
	...
	r.saveMu.Lock()
	defer r.saveMu.Unlock()
	for _, blobType := range []incremental.BlobType{incremental.DataBlob, incremental.TreeBlob} {
		if err := r.flushPacker(ctx, blobType); err != nil {
			return err
		}
	}
...
// Caller holds saveMu.
func (r *Repository) flushPacker(ctx context.Context, blobType incremental.BlobType) error {
	var packer *pack.Builder
	...
	packID := incremental.Hash(packData)
	// 1. pack first
	if err := r.backend.Save(ctx, incremental.Handle{Type: incremental.DataFile, Name: packID.String()}, bytes.NewReader(packData)); err != nil {
	...
	indexName := incremental.Hash(sealedIndex).String()
	if err := r.backend.Save(ctx, incremental.Handle{Type: incremental.IndexFile, Name: indexName}, bytes.NewReader(sealedIndex)); err != nil {
