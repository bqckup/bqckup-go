...
func Init(ctx context.Context, b backend.Backend, password string) (*Repository, error) {
	...
	if _, err := b.Stat(ctx, incremental.Handle{Type: incremental.ConfigFile}); err == nil {
	...
	config := Config{Version: 2, ID: incremental.Hash(master.Encrypt[:]), ChunkerPolynomial: polynomial}
	if err := saveConfig(ctx, b, master, config); err != nil {
		return nil, err
	}
	repo, err := newRepository(b, master, config)
	if err != nil {
		return nil, rollbackInit(ctx, b, incremental.Handle{}, err)
	}
...
func rollbackInit(ctx context.Context, b backend.Backend, key incremental.Handle, initErr error) error {
	...
	config := incremental.Handle{Type: incremental.ConfigFile}
...
func saveConfig(ctx context.Context, b backend.Backend, master *crypto.MasterKey, config Config) error {
	...
	return b.Save(ctx, incremental.Handle{Type: incremental.ConfigFile}, bytes.NewReader(sealed))
}
...
func loadConfig(ctx context.Context, b backend.Backend, master *crypto.MasterKey) (Config, error) {
	if _, err := b.Stat(ctx, incremental.Handle{Type: incremental.ConfigFile}); b.IsNotExist(err) {
		return Config{}, fmt.Errorf("%w: no config file found", incremental.ErrRepoNotFound)
	}
	var raw []byte
	...
	if err != nil {
		return Config{}, &incremental.RedactedError{
			Category: "repository",
...
func (r *Repository) saveKeyFile(ctx context.Context, password string) (incremental.Handle, error) {
	username, hostname := CurrentIdentity()
	keyFile, err := crypto.NewKeyFile(password, username, hostname, r.master, time.Now())
	if err != nil {
		return incremental.Handle{}, err
	}
	doc, err := json.Marshal(keyFile)
	if err != nil {
		return incremental.Handle{}, fmt.Errorf("repository: marshal key file: %w", err)
	}
	name := incremental.Hash(doc).String()
	handle := incremental.Handle{Type: incremental.KeyFileType, Name: name}
...
func unlockKeyFile(ctx context.Context, b backend.Backend, password string) (*crypto.MasterKey, error) {
	...
	if err := b.List(ctx, incremental.KeyFileType, func(h incremental.Handle, size int64) error {
	...
	if len(handles) == 0 {
		return nil, fmt.Errorf("%w: repository contains no key files", incremental.ErrInvalidPassword)
	}
	...
	return nil, fmt.Errorf("%w: no key file matched the given password", incremental.ErrInvalidPassword)
}
