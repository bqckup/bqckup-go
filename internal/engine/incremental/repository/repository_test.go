...
func TestInitCreatesLayout(t *testing.T) {
	...
	info, err := local.Stat(ctx, incremental.Handle{Type: incremental.ConfigFile})
	if err != nil {
		t.Fatal(err)
	}
	...
	var keyNames []string
	if err := local.List(ctx, incremental.KeyFileType, func(h incremental.Handle, size int64) error {
	...
	var keyBytes []byte
	if err := local.Load(ctx, incremental.Handle{Type: incremental.KeyFileType, Name: keyNames[0]}, 0, 0, func(rd io.Reader) error {
		keyBytes, err = io.ReadAll(rd)
		return err
	}); err != nil {
		t.Fatal(err)
	}
	if incremental.Hash(keyBytes).String() != keyNames[0] {
...
func TestInitIdempotent(t *testing.T) {
	...
	if _, err := first.SaveBlob(ctx, incremental.DataBlob, []byte("keep me")); err != nil {
		t.Fatal(err)
	}
	...
	var keyCount int
	if err := local.List(ctx, incremental.KeyFileType, func(incremental.Handle, int64) error {
		keyCount++
		return nil
	}); err != nil {
		t.Fatal(err)
	...
	if _, ok := second.index.Lookup(incremental.DataBlob, incremental.Hash([]byte("keep me"))); !ok {
...
func (b *failFirstKeySaveBackend) Save(ctx context.Context, h incremental.Handle, rd io.Reader) error {
	if h.Type == incremental.KeyFileType && !b.failed {
		b.failed = true
...
func TestOpenWrongPassword(t *testing.T) {
	...
	if !errors.Is(err, incremental.ErrInvalidPassword) {
...
func TestOpenMissingRepo(t *testing.T) {
	...
	if !errors.Is(err, incremental.ErrRepoNotFound) {
...
func TestSaveBlobDedup(t *testing.T) {
	...
	firstID, err := repo.SaveBlob(ctx, incremental.DataBlob, data)
	if err != nil {
		t.Fatal(err)
	}
	secondID, err := repo.SaveBlob(ctx, incremental.DataBlob, data)
	if err != nil {
		t.Fatal(err)
	}
	if firstID != secondID {
	...
	var packCount int
	if err := local.List(ctx, incremental.DataFile, func(incremental.Handle, int64) error {
		packCount++
		return nil
	}); err != nil {
		t.Fatal(err)
...
func TestSaveBlobKeepsDataAndTreeTypesSeparate(t *testing.T) {
	...
	repo, local := newRepo(t, ctx)
	payload := []byte(`{"nodes":[]}`)
	dataID, err := repo.SaveBlob(ctx, incremental.DataBlob, payload)
	if err != nil {
		t.Fatal(err)
	}
	treeID, err := repo.SaveBlob(ctx, incremental.TreeBlob, payload)
	if err != nil {
		t.Fatal(err)
	}
	if dataID != treeID {
	...
	dataEntry, dataOK := reopened.MasterIndex().Lookup(incremental.DataBlob, dataID)
	treeEntry, treeOK := reopened.MasterIndex().Lookup(incremental.TreeBlob, treeID)
	if !dataOK || !treeOK {
...
func TestFlushThenReopenFindsBlobs(t *testing.T) {
	ctx := context.Background()
	repo, local := newRepo(t, ctx)
	blobs := map[string]incremental.BlobType{
		"alpha": incremental.DataBlob,
		"beta":  incremental.TreeBlob,
		"":      incremental.DataBlob, // zero-length: never compressed
	}
	ids := map[string]incremental.ID{}
	...
	if err := local.List(ctx, incremental.DataFile, func(h incremental.Handle, size int64) error {
	...
	var indexCount int
	if err := local.List(ctx, incremental.IndexFile, func(incremental.Handle, int64) error {
		indexCount++
		return nil
	}); err != nil {
		t.Fatal(err)
	...
	if _, err := reopened.SaveBlob(ctx, incremental.DataBlob, []byte("alpha")); err != nil {
		t.Fatal(err)
	}
...
func TestZeroLengthBlobStoredUncompressed(t *testing.T) {
	ctx := context.Background()
	repo, local := newRepo(t, ctx)
	id, err := repo.SaveBlob(ctx, incremental.DataBlob, nil)
	if err != nil {
		t.Fatal(err)
	}
	if err := repo.Flush(ctx); err != nil {
	...
	entry, ok := repo.index.Lookup(incremental.DataBlob, id)
	if !ok {
	...
	if err := local.List(ctx, incremental.IndexFile, func(incremental.Handle, int64) error { return nil }); err != nil {
		t.Fatal(err)
	}
}
...
func TestLargeBlobFlushesPack(t *testing.T) {
	...
	if _, err := repo.SaveBlob(ctx, incremental.DataBlob, big); err != nil {
		t.Fatal(err)
	}
...
func TestLoadTreeRoundTrip(t *testing.T) {
	...
	id, err := repo.SaveBlob(ctx, incremental.TreeBlob, doc)
	if err != nil {
		t.Fatal(err)
	}
	if err := repo.Flush(ctx); err != nil {
...
func TestLoadSnapshotToleratesResticUnpackedEnvelope(t *testing.T) {
	...
	for _, plain := range [][]byte{
		...
		if err := repo.backend.Save(ctx, incremental.Handle{Type: incremental.SnapshotFile, Name: id.String()}, bytes.NewReader(sealed)); err != nil {
			t.Fatal(err)
		}
		loaded, err := repo.LoadSnapshot(ctx, id)
		if err != nil {
...
func TestLoadBlobMissingIDFails(t *testing.T) {
	ctx := context.Background()
	repo, _ := newRepo(t, ctx)
	if _, err := repo.LoadBlob(ctx, incremental.DataBlob, incremental.Hash([]byte("missing"))); err == nil {
...
func TestLoadTreeMalformedJSONFails(t *testing.T) {
	ctx := context.Background()
	repo, _ := newRepo(t, ctx)
	id, err := repo.SaveBlob(ctx, incremental.TreeBlob, []byte("[]"))
	if err != nil {
		t.Fatal(err)
	}
	if err := repo.Flush(ctx); err != nil {
