...
func (r *Repository) markTree(ctx context.Context, treeID incremental.ID, reachable map[index.BlobHandle]struct{}) error {
	treeHandle := index.BlobHandle{Type: incremental.TreeBlob, ID: treeID}
	...
	for _, node := range t.Nodes {
		...
		for _, contentID := range node.Content {
			reachable[index.BlobHandle{Type: incremental.DataBlob, ID: contentID}] = struct{}{}
		}
	}
	return nil
}
...
func (r *Repository) loadBlob(ctx context.Context, blobType incremental.BlobType, id incremental.ID) ([]byte, error) {
	...
	var sealed []byte
	err := r.backend.Load(ctx, incremental.Handle{Type: incremental.DataFile, Name: entry.PackID.String()}, int(entry.Length), int64(entry.Offset), func(rd io.Reader) error {
		var err error
...
func (r *Repository) sweep(ctx context.Context, reachable map[index.BlobHandle]struct{}, result *PruneResult) error {
	...
	packs := make(map[incremental.ID]index.Pack)
	...
	deadPacks := make([]incremental.ID, 0, len(packs))
	...
	indexed := make(map[incremental.ID]struct{}, len(packs))
...
func (r *Repository) writeIndex(ctx context.Context, keptPacks []index.Pack) error {
	...
	if err := r.backend.Save(ctx, incremental.Handle{Type: incremental.IndexFile, Name: name}, bytes.NewReader(sealed)); err != nil {
...
func (r *Repository) deletePack(ctx context.Context, packID incremental.ID, result *PruneResult) error {
