...
func fileNode(name string, size uint64, content ...incremental.ID) *Node {
	return &Node{
		Name:    name,
		Type:    TypeFile,
		Mode:    0o644,
...
func dirNode(name string, subtree *incremental.ID) *Node {
	return &Node{
		Name:    name,
		Type:    TypeDir,
...
func TestRoundTripPreservesFields(t *testing.T) {
	contentID := incremental.Hash([]byte("file content"))
	subtreeID := incremental.Hash([]byte("sub tree"))
	tr := &Tree{Nodes: []*Node{
...
func TestFieldNamesMatchRestic(t *testing.T) {
	tr := &Tree{Nodes: []*Node{fileNode("f", 3, incremental.Hash([]byte("x")))}}
	doc, err := tr.Marshal()
	if err != nil {
		t.Fatal(err)
	}
	...
	withTimes := fileNode("f", 3, incremental.Hash([]byte("x")))
