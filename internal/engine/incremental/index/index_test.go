...
func sampleIndex(t *testing.T) Index {
	t.Helper()
	packID := incremental.Hash([]byte("pack-one"))
	return Index{Packs: []Pack{{
		ID: packID,
		Blobs: []Blob{
			{ID: incremental.Hash([]byte("blob-a")), Type: incremental.DataBlob, Offset: 0, Length: 524320},
			{ID: incremental.Hash([]byte("blob-b")), Type: incremental.CompressedDataBlob, Offset: 524320, Length: 90000, UncompressedLength: 1048576},
			{ID: incremental.Hash([]byte("blob-c")), Type: incremental.TreeBlob, Offset: 614320, Length: 431},
		},
	}}}
}
...
func TestJSONShapeMatchesRestic(t *testing.T) {
	...
	if blob.Type != incremental.DataBlob {
...
func TestMasterIndexLookup10k(t *testing.T) {
	...
	const count = 10000
	entries := make(map[incremental.ID]Entry, count)
	for i := 0; i < count; i++ {
		id := incremental.Hash([]byte{byte(i), byte(i >> 8)})
		entry := Entry{Type: incremental.DataBlob, ID: id, PackID: incremental.Hash([]byte("pack")), Offset: uint32(i), Length: 100}
		entries[id] = entry
	...
	for id, want := range entries {
		got, ok := masterIndex.Lookup(incremental.DataBlob, id)
		if !ok || got != want {
			t.Fatalf("lookup %s = %+v, want %+v", id, got, want)
		}
	}
	if _, ok := masterIndex.Lookup(incremental.DataBlob, incremental.Hash([]byte("missing"))); ok {
...
func TestMasterIndexDuplicateLastWriteWins(t *testing.T) {
	masterIndex := NewMasterIndex()
	id := incremental.Hash([]byte("dup"))
	masterIndex.Add(Entry{Type: incremental.DataBlob, ID: id, PackID: incremental.Hash([]byte("pack-a")), Offset: 1})
	masterIndex.Add(Entry{Type: incremental.DataBlob, ID: id, PackID: incremental.Hash([]byte("pack-b")), Offset: 2})
	got, ok := masterIndex.Lookup(incremental.DataBlob, id)
	if !ok {
		t.Fatal("entry missing")
	}
	if got.PackID != incremental.Hash([]byte("pack-b")) || got.Offset != 2 {
	...
	for w := 0; w < writers; w++ {
		...
		go func(w int) {
			...
			for i := 0; i < perWriter; i++ {
				id := incremental.Hash([]byte{byte(w), byte(i), byte(i >> 8)})
				masterIndex.Add(Entry{Type: incremental.DataBlob, ID: id, PackID: incremental.Hash([]byte("pack")), Offset: uint32(i)})
			}
		}(w)
	}
	for r := 0; r < readers; r++ {
		...
		go func() {
			...
			for i := 0; i < writers*perWriter; i++ {
				masterIndex.Lookup(incremental.DataBlob, incremental.Hash([]byte{byte(i)}))
				masterIndex.Len()
			}
		}()
	}
...
func TestLoadAllFromBackend(t *testing.T) {
	...
	saveIndex := func(idx Index) string {
		...
		h := incremental.Handle{Type: incremental.IndexFile, Name: name}
	...
	second := Index{Packs: []Pack{{
		ID: incremental.Hash([]byte("pack-two")),
		Blobs: []Blob{
			{ID: incremental.Hash([]byte("blob-d")), Type: incremental.TreeBlob, Offset: 0, Length: 220},
		},
	}}}
	saveIndex(first)
	saveIndex(second)
	...
	for _, want := range []struct {
		id     string
		typeID incremental.BlobType
		pack   string
		offset uint32
	}{
		{"blob-a", incremental.DataBlob, "pack-one", 0},
		{"blob-b", incremental.DataBlob, "pack-one", 524320},
		{"blob-c", incremental.TreeBlob, "pack-one", 614320},
		{"blob-d", incremental.TreeBlob, "pack-two", 0},
	} {
		entry, ok := masterIndex.Lookup(want.typeID, incremental.Hash([]byte(want.id)))
		if !ok {
			t.Fatalf("blob %s not found after LoadAll", want.id)
		}
		if entry.PackID != incremental.Hash([]byte(want.pack)) || entry.Offset != want.offset {
...
func TestLoadAllCorruptIndexFails(t *testing.T) {
	...
	name := incremental.Hash(sealed).String()
	if err := repo.Save(ctx, incremental.Handle{Type: incremental.IndexFile, Name: name}, bytes.NewReader(sealed)); err != nil {
		t.Fatal(err)
	}
