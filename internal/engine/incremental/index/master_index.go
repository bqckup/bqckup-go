...
func (m *MasterIndex) Lookup(blobType incremental.BlobType, id incremental.ID) (Entry, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MasterIndex) LoadAll(ctx context.Context, b backend.Backend, master *crypto.MasterKey) error {
	...
	if err := b.List(ctx, incremental.IndexFile, func(h incremental.Handle, size int64) error {
