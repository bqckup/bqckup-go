...
type Lock struct {
	...
	handle incremental.Handle // own lock file, set by New/Refresh
...
type ErrInvalidLock struct{ Handle incremental.Handle }
...
func New(ctx context.Context, b backend.Backend, key *crypto.MasterKey, exclusive bool) (*Lock, error) {
	...
	if err := l.checkOthers(ctx, b, key, incremental.Handle{}); err != nil {
		return nil, err
	}
...
func (l *Lock) checkOthers(ctx context.Context, b backend.Backend, key *crypto.MasterKey, own incremental.Handle) error {
	var err error
	for attempt := 0; attempt < 3; attempt++ {
		err = b.List(ctx, incremental.LockFile, func(h incremental.Handle, _ int64) error {
			if h.Name == own.Name {
				return nil // own lock
			}
...
func loadLock(ctx context.Context, b backend.Backend, key *crypto.MasterKey, h incremental.Handle) (*Lock, error) {
	l, _, err := loadLockTyped(ctx, b, key, h)
	return l, err
}

func loadLockTyped(ctx context.Context, b backend.Backend, key *crypto.MasterKey, h incremental.Handle) (*Lock, bool, error) {
	var raw []byte
