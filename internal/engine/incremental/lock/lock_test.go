...
func writeLock(t *testing.T, b backend.Backend, key *crypto.MasterKey, name string, l Lock) {
	...
	l.handle = incremental.Handle{Type: incremental.LockFile, Name: name}
...
func listLockNames(t *testing.T, b backend.Backend) []string {
	t.Helper()
	var names []string
	err := b.List(context.Background(), incremental.LockFile, func(h incremental.Handle, _ int64) error {
...
func TestLockBlobRoundTrip(t *testing.T) {
	...
	h := incremental.Handle{Type: incremental.LockFile, Name: incremental.Hash(blob).String()}
	...
	assert.Equal(t, incremental.Hash(blob).String(), h.Name)
...
func TestInvalidLockFileBlocks(t *testing.T) {
	b := testBackend(t)
	key := testKey(t)
	require.NoError(t, b.Save(context.Background(), incremental.Handle{Type: incremental.LockFile, Name: strings.Repeat("e", 64)}, strings.NewReader("not a sealed lock")))
...
func (b *failRemoveBackend) Remove(ctx context.Context, h incremental.Handle) error {
	if b.failures > 0 {
		b.failures--
...
func (b *cancelAfterSaveBackend) Save(ctx context.Context, h incremental.Handle, rd io.Reader) error {
...
func TestRemoveStaleRemovesOnlyStaleLocks(t *testing.T) {
	...
	require.NoError(t, b.Save(context.Background(), incremental.Handle{Type: incremental.LockFile, Name: strings.Repeat("c", 64)}, strings.NewReader("garbage")))
