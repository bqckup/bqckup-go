...
func backupTestFiles(t *testing.T, r *Repository, files map[string]string, tags []string) {
	...
	for _, name := range names {
		content := files[name]
		id, err := r.SaveBlob(ctx, restic.DataBlob, []byte(content))
		require.NoError(t, err)
		nodes = append(nodes, &tree.Node{
			Name: name, Type: tree.TypeFile, Mode: 0o600, ModTime: time.Now(),
			UID: 1000, GID: 1000, Size: uint64(len(content)), Content: []restic.ID{id},
		})
	}
	doc, err := (&tree.Tree{Nodes: nodes}).Marshal()
	require.NoError(t, err)
	treeID, err := r.SaveBlob(ctx, restic.TreeBlob, doc)
	require.NoError(t, err)
...
func packHandles(t *testing.T, b backend.Backend) []restic.Handle {
	t.Helper()
	var packs []restic.Handle
	err := b.List(context.Background(), restic.DataFile, func(h restic.Handle, _ int64) error {
...
func indexHandles(t *testing.T, b backend.Backend) []restic.Handle {
	t.Helper()
	var indexes []restic.Handle
	err := b.List(context.Background(), restic.IndexFile, func(h restic.Handle, _ int64) error {
...
func TestPartiallyReferencedPackSurvives(t *testing.T) {
	...
	blobX, err := r.SaveBlob(ctx, restic.DataBlob, []byte("content-x"))
	require.NoError(t, err)
	blobY, err := r.SaveBlob(ctx, restic.DataBlob, []byte("content-y"))
	require.NoError(t, err)
...
func TestPruneRemovesOrphanedPacks(t *testing.T) {
	...
	require.NoError(t, b.Save(ctx, restic.Handle{Type: restic.DataFile, Name: strings.Repeat("9", 64)}, strings.NewReader("orphan bytes")))
...
func TestPruneWithoutForgetStillCleansGarbage(t *testing.T) {
	...
	backupTestFiles(t, r, map[string]string{"f.txt": "data"}, []string{"bqckup", "site:test"})
	require.NoError(t, b.Save(ctx, restic.Handle{Type: restic.DataFile, Name: strings.Repeat("8", 64)}, strings.NewReader("garbage")))
...
func diffPacks(newer, older []restic.Handle) []restic.Handle {
	out := make([]restic.Handle, 0)
	for _, h := range newer {
		found := false
		for _, o := range older {
			if o.Name == h.Name {
...
func saveTestTree(t *testing.T, r *Repository, contentID restic.ID) restic.ID {
	t.Helper()
	doc, err := (&tree.Tree{Nodes: []*tree.Node{{
		Name: "f", Type: tree.TypeFile, Mode: 0o600, ModTime: time.Now(),
		UID: 1000, GID: 1000, Size: 1, Content: []restic.ID{contentID},
	}}}).Marshal()
	require.NoError(t, err)
	id, err := r.SaveBlob(context.Background(), restic.TreeBlob, doc)
	require.NoError(t, err)
...
func verifyIndexesReferenceExistingPacks(ctx context.Context, r *Repository, b backend.Backend) error {
	return b.List(ctx, restic.IndexFile, func(h restic.Handle, _ int64) error {
		var raw []byte
		...
		for _, pack := range idx.Packs {
			if _, err := b.Stat(ctx, restic.Handle{Type: restic.DataFile, Name: pack.ID.String()}); err != nil {
				return err
			}
		}
		return nil
