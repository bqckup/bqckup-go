...
func parseHeader(header []byte) ([]Blob, error) {
	...
	for position < len(header) {
		entryType := restic.BlobType(header[position])
		position++
		compressed := entryType == restic.CompressedDataBlob || entryType == restic.CompressedTreeBlob
		if entryType > restic.CompressedTreeBlob {
			return nil, fmt.Errorf("pack: unknown blob type %d at header offset %d", entryType, position-1)
		}
		if !compressed && len(header)-position < 4+len(restic.ID{}) {
			return nil, errors.New("pack: truncated header entry")
		}
		if compressed && len(header)-position < 8+len(restic.ID{}) {
