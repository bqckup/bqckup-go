...
func (b *Builder) Add(blobType restic.BlobType, id restic.ID, ciphertext []byte, uncompressedLength uint32) {
...
func buildHeader(blobs []Blob) ([]byte, error) {
	...
	for _, blob := range blobs {
		...
		if blob.Type == restic.CompressedDataBlob || blob.Type == restic.CompressedTreeBlob {
			size += 4
		}
	}
