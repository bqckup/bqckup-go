...
func addBlob(t *testing.T, b *Builder, master *crypto.MasterKey, blobType restic.BlobType, plaintext []byte) {
	t.Helper()
	...
	if blobType == restic.CompressedDataBlob || blobType == restic.CompressedTreeBlob {
		uncompressedLength = uint32(len(plaintext))
	}
	b.Add(blobType, restic.Hash(plaintext), sealed, uncompressedLength)
}
...
func TestBuildParseRoundTrip(t *testing.T) {
	...
	types := []restic.BlobType{restic.DataBlob, restic.CompressedDataBlob, restic.TreeBlob}
	...
	for i, blob := range blobs {
		...
		if blob.ID != restic.Hash(plaintexts[i]) {
		...
		if types[i] == restic.CompressedDataBlob && blob.UncompressedLength != uint32(len(plaintexts[i])) {
...
func TestHeaderTrailerValidated(t *testing.T) {
	master := testMaster(t)
	b := NewBuilder()
	addBlob(t, b, master, restic.DataBlob, []byte("payload"))
	packData, err := b.Finalize(master)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCorruptedHeaderRejected(t *testing.T) {
	master := testMaster(t)
	b := NewBuilder()
	addBlob(t, b, master, restic.DataBlob, bytes.Repeat([]byte{7}, 4096))
	packData, err := b.Finalize(master)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLargeBlobRoundTrip(t *testing.T) {
	...
	addBlob(t, b, master, restic.CompressedTreeBlob, big)
	packData, err := b.Finalize(master)
	if err != nil {
		t.Fatal(err)
	}
