...
func NewAdapter(process ProcessRunner) *Adapter {
	if process == nil {
		process = NewProcessRunner()
	}
	return &Adapter{process: process}
}

func (a *Adapter) Preflight() error {
	if _, err := a.process.LookPath("restic"); err != nil {
		return hiddenError("required restic binary is unavailable in PATH", err)
	}
	return nil
}
...
func (a *Adapter) EnsureRepository(ctx context.Context, repo RepoConfig) error {
	...
	err := a.process.Run(ctx, ProcessSpec{
		Command: "restic",
		Args:    args,
		Env:     a.env(repo),
		Stdout:  &stdout,
	...
	if err != nil {
		...
		return hiddenError(fmt.Sprintf("could not initialize restic repository: %s", sanitize(errOutput)), err)
	}
	return nil
}
...
func (a *Adapter) BackupFiles(ctx context.Context, repo RepoConfig, spec BackupSpec) (SnapshotSummary, error) {
	...
	if err != nil {
		...
		return SnapshotSummary{}, hiddenError(fmt.Sprintf("could not create restic snapshot: %s", sanitize(stderr.String())), err)
	}

	summary, parseErr := parseSnapshotSummary(stdout.Bytes())
	if parseErr != nil {
		return SnapshotSummary{}, hiddenError("could not parse restic summary", parseErr)
	}
	return summary, nil
}

func (a *Adapter) ApplyRetention(ctx context.Context, repo RepoConfig, keepLast int, siteName string) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	if err := a.Preflight(); err != nil {
	...
	if err != nil {
		...
		return hiddenError(fmt.Sprintf("could not apply restic retention: %s", sanitize(stderr.String())), err)
	}
	return nil
}
...
func (a *Adapter) Unlock(ctx context.Context, repo RepoConfig) error {
	...
	if err != nil {
		...
		return hiddenError(fmt.Sprintf("could not unlock restic repository: %s", sanitize(stderr.String())), err)
	}
	return nil
}
...
func (h *hiddenErr) Error() string { return h.public }
func (h *hiddenErr) Unwrap() error { return h.cause }

func hiddenError(public string, cause error) error {
	return &hiddenErr{public: public, cause: cause}
}
